...
À nossas famílias, pelo apoio e compreensão.
\end{agradecimentos}

\begin{resumo}
Aprendizado por reforço permite que um agente aprenda a realizar tarefas de forma otimizada através de experiências obtidas de interações com o ambiente. Este texto apresenta a contextualização do assunto e busca detalhar este conceito a ser aplicado em navegação robótica.
\end{resumo}

\begin{abstract}
Reinforcement learning allows an agent learn to perform tasks optimally through experiences gained from interactions with the environment. This paper presents the context of the subject and aims to detail this concept to be applied to robotic navigation.
\end{abstract}
...
o processo de aprendizado é muito lento. Com o objetivo de contornar este problema, propõe-
se o uso de formas de transferência do conhecimento aprendido em tarefas anteriores,
expresso por políticas de atuações, em novas tarefas similares com as quais o agente se
depara, visando, com isso, que o agente demonstre um comportamento adequado mesmo
estando no início do processo de aprendizagem. Conforme seu conhecimento sobre a tarefa
atual aumenta, com a evolução de sua experiência e do processo de aprendizagem, sua
atuação ficará cada vez melhor.
...
\begin{figure}[!h]
    ...
    \caption{Modelo padrão de Aprendizado por Reforço -- neste modelo tem-se o agente B interagindo com o ambiente T; o agente percebe o estado atual s através da entrada i, executa uma ação a, observa o novo estado e recebe uma recompensa r.}
...
ditam como essa política ? deve mudar conforme os resultados da experiência do agente são
contabilizados.
...
A seguir introduzimos o conceito de \emph{Processos de Decisão de Markov} (PDM), que
...
\subsubsection{On-Policy SARSA-Learning}
...
\begin{figure}[!h]
    ...
    \caption{Abstração utilizada para compreensão de onde exatamente encontram-se o valor-estado e o valor-ação.}
...
\begin{figure}[!h]
    ...
    \caption{Modelo formado por estados e transições considerados em métodos por DT de 1 único
...
\begin{figure}[!h]
    ...
    \caption{Diagrama de métodos por DT($\lambda$) com atribuição de pesos a cada estrutura de n passos.}
...
Dado esse novo conceito de retorno ?, a equação (\ref{eq:incremento_valor_estado}), que representa a atualização dos valores-estado, pode ser alterada para a expressão (\ref{eq:incremento_valor_estado_lambda}) que considera a média de retornos ao invés de um número específico n de passos.
...
\begin{figure}[!h]
    ...
    \caption{Ilustração de como funciona a visão adiante -- o agente deve olhar apenas o panorama
...
\begin{figure}[!h]
    ...
    \caption{Gráfico explicativo sobre como o valor do traço de elegibilidade varia caso ocorram visitas a
um mesmo estado.}
...
\begin{figure}[!h]
    ...
    \caption{Ilustração de como funciona a visão retrógrada de métodos por DT -- o agente deve passar
aos estados anteriores a sua situação atual.}
...
Conforme apresentado na seção \ldots \\
\vdots
...
\begin{figure}[!h]
    ...
    \caption{Simulador TurtleSim sendo executado. Aqui o rono está fazendo uma rota circular, assim
como no trajeto desenhado.}
    \label{fig:turtlesim}
\end{figure}
...
\begin{figure}[!h]
    ...
    \caption{Sistema robótico a ser utilizado neste projeto (Pioneer 2DX) -- neste exemplo o robô já
possui uma câmera auxiliar e podemos visualizar os sensores acoplados ao robô.}
